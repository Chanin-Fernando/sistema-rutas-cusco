    ...
    def asignar_pedidos(self, pedidos: List[Pedido]) -> tuple:
        ...
        for pedido in pedidos:
            ...
            for rep in self.repartidores:
                ...
                if capacidad_restante < pedido.peso:
                    continue
                dist = rep.distancia_a_pedido(pedido)
                if dist < mejor_dist:
                    mejor_dist = dist
                    mejor_rep = rep
            ...
            if mejor_rep:
                ...
                self.log_pasos.append(
                    ...
                    f"{mejor_rep.nombre} | dist={mejor_dist:.1f} | "
        ...
        for rep in self.repartidores:
            ...
            if not pedidos_rep:
                rutas[rep.id] = []
                continue
            rutas[rep.id] = self._vecino_mas_cercano(rep.pos_x, rep.pos_y, pedidos_rep)

        return asignaciones, rutas

    def _vecino_mas_cercano(self, ox: float, oy: float, pedidos: List[Pedido]) -> List[Pedido]:
        pendientes = list(pedidos)
        ruta = []
        cx, cy = ox, oy
        while pendientes:
            pendientes.sort(key=lambda p: p.distancia_a(cx, cy))
